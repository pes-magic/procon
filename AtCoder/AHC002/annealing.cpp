...
int main(int argc, const char* argv[]){
    const double initial_temp = 0;
    const double final_temp = 150;

    // if(argc >= 2) initial_temp = atof(argv[1]);
    // if(argc >= 3) final_temp = atof(argv[2]);
    ...
    const double first_time_limit = timer.msec();
    const double time_limit = 1900.0;
    int wholeBestScore = bestScore;
    vector<pair<int, int>> wholeBestStep = bestStep;
    while(timer.msec() < time_limit){
        ...
        const double ratio = (timer.msec() - first_time_limit) / (time_limit - first_time_limit);
        ...
        double delta = midBestScore - curBest;
        ...
        if(delta > 0 || bernoulli_distribution(exp(delta/temperature))(rnd)){
            ...
            bestScore += midBestScore - curBest;
            if(bestScore > wholeBestScore){
                wholeBestScore = bestScore;
                wholeBestStep = bestStep;
            }
        }
    }
    bestStep = wholeBestStep;
